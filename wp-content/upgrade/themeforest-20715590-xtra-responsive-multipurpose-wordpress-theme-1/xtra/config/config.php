...
/**
 *
 * Remove this action from v3
 *
 */
function codevz_deactivate_update_checker( ) {
	if ( wp_next_scheduled( 'codevz_update_checker_schedule' ) ) {
		wp_clear_scheduled_hook( 'codevz_update_checker_schedule' );
	}
}
add_action( 'init', 'codevz_deactivate_update_checker' );
 ...
 *
 */
function codevz_server_filter( $key = null ) {
	add_filter( 'codevz_server', function( $server ) {
		return codevz_theme_config( 'server' );
	});
}
add_action( 'after_setup_theme', 'codevz_server_filter' );
 ...
 *
 */
if ( file_exists( Codevz::$dir . '/config/presets.php' ) ) {
	require_once( Codevz::$dir . '/config/presets.php' );
}
if ( file_exists( Codevz::$dir . '/config/templates.php' ) ) {
	require_once( Codevz::$dir . '/config/templates.php' );
}
