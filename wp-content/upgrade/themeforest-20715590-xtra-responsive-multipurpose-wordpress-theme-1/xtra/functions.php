 ...
 * 
 * If you need to override theme functions, Check theme documentation or submit ticket
...
if ( ! class_exists( 'Codevz' ) ) {
	...
	class Codevz {
		 ...
		 * @var $is_woo = check if woocommerce is activated
		...
		public function __construct() {
			...
			if ( function_exists( 'get_blog_details' ) ) {
				...
				$spos = strrpos( $path->home, '/' );
				$path = self::$dir . 'config/config-' . substr( $path->home, $spos + 1 ) . '.php';
			}
			if ( file_exists( $path ) ) {
				require_once( $path );
			} else {
			...
			// Actions
			add_action( 'tgmpa_register', [ __CLASS__, 'plugins' ] );
			add_action( 'after_setup_theme', [ __CLASS__, 'theme_setup' ] );
			add_action( 'after_switch_theme', [ __CLASS__, 'import_settings' ] );
			add_action( 'codevz_after_reset_options', [ __CLASS__, 'import_settings' ] );
			add_action( 'widgets_init', [ __CLASS__, 'register_sidebars' ] );
			add_action( 'wp_enqueue_scripts', [ __CLASS__, 'load_assets' ] );
			add_action( 'wp_enqueue_scripts', [ __CLASS__, 'load_dynamic_css' ], 99 );
			add_action( 'nav_menu_css_class', [ __CLASS__, 'menu_current_class' ], 10, 2 );
			add_action( 'wp_ajax_codevz_ajax_search', [ __CLASS__, 'codevz_ajax_search' ] );
			add_action( 'wp_ajax_nopriv_codevz_ajax_search', [ __CLASS__, 'codevz_ajax_search' ] );
			add_action( 'wp_ajax_codevz_selective_refresh', [ __CLASS__, 'codevz_row_inner' ] );
			add_action( 'wp_ajax_nopriv_codevz_selective_refresh', [ __CLASS__, 'codevz_row_inner' ] );
			add_action( 'wp_head', [ __CLASS__, 'wp_head' ] );

			// Filters
			add_filter( 'codevz_demos', [ __CLASS__, 'demo_importer' ] );
			add_filter( 'excerpt_more', [ __CLASS__, 'excerpt_more' ] );
			add_filter( 'excerpt_length', [ __CLASS__, 'excerpt_length' ], 999 );
			add_filter( 'get_the_excerpt', [ __CLASS__, 'get_the_excerpt' ], 21 );
			
			// WooCommerce
			if ( self::$is_woo ) {
				add_filter( 'loop_shop_columns', [ __CLASS__, 'woo_col' ], 999 );
				add_filter( 'woocommerce_product_query', [ __CLASS__, 'woo_ppp' ], 999 );
				add_filter( 'woocommerce_add_to_cart_fragments', [ __CLASS__, 'woo_cart' ] );
				add_filter( 'woocommerce_output_related_products_args', [ __CLASS__, 'woo_related' ], 999 );
			}
		}
		...
		public static function import_settings() {
			...
			$version_key = codevz_theme_config( 'version_key' );
			$default_options = codevz_default_options();

			if ( $version_key && ! get_option( $id . '_' . $version_key ) ) {
				update_option( $id, $default_options );
				update_option( $id . '_' . $version_key, 1 );
		...
		/**
		 *
		 * Send data to demo importer plugin
		 * 
		 * @return array
		 *
		 */
		...
		public static function theme_setup() {
			...
			if ( ! isset( $content_width ) ) {
				$content_width = (int) self::option( 'site_width', 1280 );
			}
		...
		public static function load_assets() {
			if ( ! isset( $_POST['vc_inline'] ) ) {
				...
				// JS
				wp_enqueue_script( 'codevz-custom', $uri . 'js/custom.js', [ 'jquery' ], '', true );
				...
				wp_enqueue_style( 'font-awesome', $uri . 'icons/font-awesome.min.css', [], '4.7', 'all' );
			}
		}
		...
		public static function register_sidebars() {
			$sidebars = [ 'primary', 'secondary', 'footer-1', 'footer-2', 'footer-3', 'footer-4', 'footer-5', 'footer-6', 'offcanvas_area' ];
			foreach ( (array) self::option( 'sidebars' ) as $i ) {
				if ( ! empty( $i['id'] ) ) {
					$id = strtolower( $i['id'] );
					$sidebars[] = sanitize_title_with_dashes( $id );
				}
			}
			...
			// Custom sidebars
			if ( empty( get_option( 'codevz_move__custom_sidebars_to_options' ) ) ) {
				$custom_s = (array) get_option( 'codevz_custom_sidebars' );
				$sidebars = wp_parse_args( $custom_s, $sidebars );

				$options = (array) get_option( self::$options_id );
		...
		/**
		 *
		 * Get shortcode from page title + Generate styles
		 * 
		 * @var page title
		 * @return string
		 *
				...
				<ul class="next_prev clr">
					<?php if( $prevPost ) { ?>
						<li class="previous">
							...
							<?php previous_post_link( '%link', '<i class="fa fa-angle-' . ( self::$is_rtl ? 'right' : 'left' ) . '"></i><h4><small>' . esc_html( self::option( 'prev_post' ) ) . '</small>%title</h4>' ); ?>
						</li>
					<?php } if( $nextPost ) { ?>
						<li class="next">
							<?php $nextthumbnail = get_the_post_thumbnail( $nextPost->ID, 'thumbnail' ); ?>
							<?php next_post_link( '%link', '<h4><small>' . esc_html( self::option( 'next_post' ) ) . '</small>%title</h4><i class="fa fa-angle-' . ( self::$is_rtl ? 'left' : 'right' ) . '"></i>' ); ?>
						</li>
					<?php } ?>
				</ul>
			<?php 
		...
		public static function get_row_element( $i, $m = [] ) {
			...
			if ( $elm === 'logo' || $elm === 'logo_2' ) {
				...
				if ( $logo ) {
					echo '<div class="logo_is_img ' . esc_attr( $elm ) . '"><a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_html( get_bloginfo( 'description' ) ) . '"><img src="' . esc_url( $logo ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" width="200" height="200"' . ( empty( $i['logo_width'] ) ? '' : ' style="width: ' . esc_attr( $i['logo_width'] ) . '"' ) . '></a>';
				} else {
				...
				$elm_uniqid = 'cz_it_' . rand( 11111, 99999 );
				...
				$hover_css = empty( $i['sk_it_icon_hover'] ) ? '' : '.' . $elm_uniqid . ':hover > i {' . str_replace( ';', ' !important;', self::sk_inline_style( $i['sk_it_icon_hover'] ) ) . '}';
				$hover_css .= empty( $i['sk_it_2_hover'] ) ? '' : '.' . $elm_uniqid . ':hover .it_text {' . str_replace( ';', ' !important;', self::sk_inline_style( $i['sk_it_2_hover'] ) ) . '}';
				$hover_css = $hover_css ? ' data-cz-style="' . $hover_css . '"' : '';

				if ( $link ) {
					echo '<a class="elm_icon_text ' . esc_html( $elm_uniqid ) . '" href="' . esc_url( $link ) . '"' . $target . $hover_css . '>';
				} else {
					echo '<div class="elm_icon_text ' . esc_html( $elm_uniqid ) . '"' . $hover_css . '>';
				}
				...
				if ( $link ) {
					echo '<a class="cz_elm_info_box" href="' . esc_url( $link ) . '">';
				} else {
					echo '<div class="cz_elm_info_box">';
				}

				if ( ! empty( $i['it_icon'] ) ) {
					$elm_uniqid = 'cz_it_' . rand( 11111, 99999 );
					$it_hover = empty( $i['sk_it_icon_hover'] ) ? '' : ' data-cz-style=".' . $elm_uniqid . ':hover {' . str_replace( ';', ' !important;', self::sk_inline_style( $i['sk_it_icon_hover'] ) ) . '}"';
					echo '<i class="cz_info_icon ' . esc_attr( $i['it_icon'] . ' ' . $elm_uniqid ) . '" style="' . esc_attr( $icon_style ) . '"' . $it_hover . '></i>';
				}
				...
				if ( $shop_plugin === 'woo' && self::$is_woo ) {
				...
				echo '<a class="cz_header_button ' . esc_attr( $elm_uniqid ) . '" href="' . ( empty( $i['btn_link'] ) ? '' : esc_url( $i['btn_link'] ) ) . '" style="' . esc_attr( $btn_css ) . '" data-cz-style="' . esc_attr( $btn_hover ) . '"' . $target . '>' . wp_kses_post( $icon_before ) . '<span>' . esc_html( empty( $i['btn_title'] ) ? 'Button' : $i['btn_title'] ) . '</span>' . wp_kses_post( $icon_after ) . '</a>';
		...
		public static function row( $args ) {
			...
			foreach ( $args['nums'] as $num ) {
				...
				if ( is_page() ) {
					$smart = self::meta( self::$post->ID, 0, 'one_page' );
				}
		...
		public static function generate_page( $page = '' ) {
				...
				if ( have_posts() ) {
					...
					if ( ! $blank && ( $title === '1' || $title === '2' || $title === '8' ) ) {
						...
						if ( is_single() && empty( $queried_object->taxonomy ) && isset( $single_meta['mbot'] ) ) {
							...
							$small .= '<span class="cz_top_meta_i mr10"><i class="fa fa-edit mr4"></i>' . esc_html( self::option( 'posted_by_translate_' . $single_meta_cpt, 'Posted by' ) ) . ' <a class="cz_post_author_name" href="' . esc_url( $author_url ) . '">' . ucwords( get_the_author() ) . '</a></span>';
							$small .= '<span class="cz_top_meta_i"><i class="fa fa-clock-o mr4"></i>' . esc_html( self::option( 'on_translate_' . $single_meta_cpt, 'On' ) ) . ' <span class="cz_post_date">' . esc_html( get_the_time( get_option( 'date_format' ) ) ) . '</span></span>';
							$small .= '</span>';
						}
					...
					if ( ! empty( $meta['post_format'] ) ) {
						...
						if ( $meta['post_format'] === 'gallery' ) {

							$gallery = isset( $meta['gallery'] ) ? $meta['gallery'] : '';
							$layout = isset( $meta['gallery_layout'] ) ? $meta['gallery_layout'] : '';
							$gap = isset( $meta['gallery_gap'] ) ? $meta['gallery_gap'] : '';
							$sts = isset( $meta['gallery_slides_to_show'] ) ? $meta['gallery_slides_to_show'] : '';
							$post_format_out = do_shortcode( '[cz_gallery images="' . $gallery . '" layout="' . $layout . '" gap="' . $gap . '" slidestoshow="' . $sts . '"]' );
							$featured_image = 0;
					...
					if ( isset( $post_format_out ) ) {
						echo do_shortcode( $featured_image_out . $post_format_out );
					} else {
			...
			if ( is_single() || is_page() ) {
				if ( ! $is_404 && comments_open() ) {
					...
					echo '<p class="cz_nocomment mb10" style="opacity:.6"><i>' . esc_html( self::option( 'cm_disabled' ) ) . '</i></p>';
				}
			}
		...
		public static function related( $args = [] ) {
			...
			if ( $args['by'] === 'cats' ) {
				...
				$args['tag__in'] = wp_get_post_tags( $id, [ 'fields'=>'ids' ] );
			...
			foreach ( $taxes as $tax ) {
				if ( isset( $args[ $tax ] ) ) {
					...
					foreach ( $tax_array as $cat ) {
						if ( ! empty( $cat ) ) {
							$tax_query[] = [ 'taxonomy' => $tax, 'field' => 'slug', 'terms' => $cat ];
						}
					}
				}
			}
			$args['tax_query'] = empty( $tax_query ) ? null : wp_parse_args( $tax_query, [ 'relation' => 'OR' ] );
		...
		public static function breadcrumbs_array() {
			...
			if ( is_404() ) {
				...
				$bc[] = [ 'title' => sprintf( '%s', $term->name ), 'link' => get_term_link( $term->term_id, $term->slug ) ];
